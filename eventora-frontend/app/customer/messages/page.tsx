...
import { MessageCircle, Send, Search, Phone, Video, MoreVertical, Bell } from 'lucide-react'
  ...
  useEffect(() => {
    ...
    const handleReceive = (msg: Message) => {
      ...
      if (conv && msg.conversationId === conv.id) {
        ...
        const senderName = senderConv?.vendorName || 'New message'
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredConvs.length === 0 ? (
                ...
                filteredConvs.map(conv => (
                  ...
                  >
                    {/* Avatar */}
                    <div className={`w-11 h-11 rounded-full flex-shrink-0 flex items-center justify-center text-white text-sm font-semibold ${avatarColor(conv.vendorName)}`}>
                      {initials(conv.vendorName)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline">
                        <p className="text-sm font-semibold text-foreground truncate">{conv.vendorName}</p>
            ...
            {selected ? (
              <>
                ...
                <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card">
                  ...
                  <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center text-white text-sm font-semibold ${avatarColor(selected.vendorName)}`}>
                    {initials(selected.vendorName)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-foreground text-sm">{selected.vendorName}</p>
                ...
                >
                  {messages.length === 0 ? (
                    ...
                    groupedMessages.map(({ dateSep, msg }, idx) => (
                      <div key={msg.id}>
                        {dateSep && (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-sm text-muted-foreground max-w-xs mb-6">
                  Select a conversation from the list, or book a vendor to start chatting.
                </p>
                <Button asChild variant="outline"><Link href="/vendors">Browse Vendors</Link></Button>
              </div>
            )}
          </div>
        </div>
